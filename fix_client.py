...
class CoinbaseFIXClient:
    ...
    async def connect(self, max_retries: int = None) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                if attempt > 1:
                    logger.info(
                        f"Connection attempt {attempt}/{max_retries} for {self.session_type} session"
                    )
                ...
                if self.journaler:
                    try:
                        ...
                        logger.warning(
                            f"Error closing previous journaler connection: {e}"
                        )
                ...
                try:
                    if self.test_mode:
                        self.connected = True
                        logger.info(
                            f"[TEST MODE] Simulated connection to {self.host}:{self.port} for {self.session_type} session"
                        )
                    else:
                        ...
                        self.connected = True
                        logger.info(
                            f"Connected to {self.host}:{self.port} for {self.session_type} session"
                        )
                        ...
                        if not auth_success:
                            logger.error(
                                f"Failed to authenticate for {self.session_type} session"
                            )
                    ...
                    if "SSL" in str(e) or "TLS" in str(e):
                        logger.error(
                            "SSL/TLS handshake failed. Check your certificates."
                        )
                    if "Connection refused" in str(e):
                        logger.error(
                            "Connection refused. Your IP may not be whitelisted."
                        )
                    if "timeout" in str(e).lower():
                        logger.error(
                            "Connection timeout. Your IP may not be whitelisted or there may be network issues."
                        )
                    raise
                ...
                if self.test_mode:
                    ...
                    self.authenticated = True
                    logger.info(
                        f"[TEST MODE] Simulated successful authentication for {self.session_type} session"
                    )
                    return True
                ...
                while (
                    not self.authenticated and time.time() - auth_start < auth_timeout
                ):
                ...
                logger.error(
                    f"Error connecting to FIX server (attempt {attempt}/{max_retries}): {e}"
                )
                self.connected = False
                self.authenticated = False
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            if (
                self.journaler
                and hasattr(self.journaler, "conn")
                and self.journaler.conn
            ):
                try:
                    self.journaler.conn.close()
                    logger.debug(
                        f"Closed journaler connection for {self.session_type} session"
                    )
                except Exception as e:
    ...
    async def _authenticate(self) -> bool:
        ...
        try:
            ...
            if self.test_mode:
                logger.info(
                    f"[TEST MODE] Simulated authentication for {self.session_type} session"
                )
                return True
            ...
            # Signature format: timestamp + api_key + target_comp_id + passphrase
            message = (
                f"{utc_timestamp}{self.api_key}{self.target_comp_id}{self.passphrase}"
            )
            logger.info(
            ...
            try:
                try:
                    ...
                    logger.info(
                        f"Decoded API secret length: {len(decoded_secret)} bytes"
                    )
                except Exception as e:
            ...
            logon_msg.set(
                95, str(len(signature))
            )  # RawDataLength: Length of signature (Tag 95)
            logon_msg.set(96, signature)  # RawData: HMAC signature (Tag 96)
            logon_msg.set(
                58, utc_timestamp
            )  # Text: Timestamp used for signature (Tag 58)
            ...
            if self.session_type == "order_entry":
                logon_msg.set(
                    8013, "N"
                )  # CancelOrdersOnDisconnect: Don't cancel orders on disconnect
                logon_msg.set(
                    8014, "N"
                )  # CancelOrdersOnInternalDisconnect: Don't cancel on internal disconnect
    ...
    def _handle_reject(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Session level Reject: RefSeqNum={ref_seq_num}, "
    ...
    def _handle_business_reject(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Business Reject: RefSeqNum={ref_seq_num}, "
    ...
    def _log_execution_report(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"ExecutionReport: {clord_id} {symbol} {side} {ord_type_desc} - {exec_desc} ({status_desc})"
            )
            ...
            if exec_type in ["1", "2"]:  # Partial Fill or Fill
                try:
                    ...
                    if no_misc_fees > 0 and self.test_mode:
                        ...
                        fee_type_desc = fee_type_map.get(
                            fee_type, f"Unknown ({fee_type})"
                        )
    ...
    def _handle_order_mass_cancel_report(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            response_map = {"0": "Cancel Request Rejected", "7": "Cancel All Orders"}
            ...
            if mass_cancel_response == "0":
                ...
                logger.info(
                    f"Mass Cancel Report: {clord_id} {symbol} {side} - {response_desc}, "
    ...
    def _handle_order_cancel_reject(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Order Cancel Reject: {clord_id} (orig: {orig_clord_id}) - "
    ...
    def _handle_trade_capture_report(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            trd_type_map = {"0": "Regular trade", "3": "Transfer"}
            ...
            if trd_type == "3":  # Transfer
                ...
                logger.info(
                    f"Trade Capture Report: {trd_match_id} {symbol} {side} - "
                ...
                if fees:
                    ...
                    for fee in fees:
                        logger.info(
                            f"    {fee['amount']} {fee['currency']} ({fee['type']})"
                        )
            else:
                logger.info(
                    f"Trade Capture Report: {trd_match_id} {symbol} {side} - "
    ...
    def _handle_prefill_report(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            liquidity_map = {"1": "Added liquidity", "2": "Removed liquidity"}

            liquidity_desc = liquidity_map.get(last_liquidity_ind, last_liquidity_ind)

            logger.info(
                f"PreFill Report: {clord_id} {symbol} {side} - "
    ...
    def _handle_quote_request(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"Quote Request: {quote_req_id} {symbol} - "
        ...
        try:
            ...
            if quote_status == "19":  # Pending End Trade
                ...
                if side and order_qty:
                    logger.info(
                        f"  Quote selected for execution: {side_desc} {order_qty}"
                    )
            ...
            if quote_status == "5":  # Rejected
                text = message.get(FTag.Text, "")
                logger.warning(
                    f"Quote Status Report: {quote_id} {symbol} - {status_desc}: {text}"
                )
            elif quote_status == "7":  # Expired
                logger.warning(
                    f"Quote Status Report: {quote_id} {symbol} - {status_desc}, "
    ...
    async def send_prefill_request(self, prefill_request_id: str = None) -> str:
        ...
        if not self.authenticated or self.session_type != "order_entry":
            logger.error(
                "Cannot send prefill request: Not authenticated or wrong session type"
            )
            return ""
    ...
    async def send_rfq_request(self, rfq_req_id: str = None) -> str:
        ...
        if not self.authenticated or self.session_type != "order_entry":
            logger.error(
                "Cannot send RFQ request: Not authenticated or wrong session type"
            )
            return ""
    ...
    async def subscribe_market_data(self, symbol: str, depth: int = 10) -> bool:
        ...
        if not self.authenticated or self.session_type != "market_data":
            logger.error(
                "Cannot subscribe to market data: Not authenticated or wrong session type"
            )
            return False
    ...
    async def unsubscribe_market_data(self, symbol: str) -> bool:
        ...
        if not self.authenticated or self.session_type != "market_data":
            logger.error(
                "Cannot unsubscribe from market data: Not authenticated or wrong session type"
            )
            return False

        if self.test_mode:
            logger.info(
                f"Test mode: Simulating market data unsubscription for {symbol}"
            )
            return True
    ...
    async def place_order(
        ...
        if self.test_mode:
            ...
            logger.info(
                f"Test mode: Simulating {order_type} {side} order for {quantity} {symbol} at price {price}"
            )
        ...
        try:
            ...
            if order_type.upper() == "TAKE_PROFIT_STOP_LOSS":
                if time_in_force.upper() not in ["GTC", "GTD"]:
                    raise ValueError(
                        "TPSL orders only support GTC and GTD time in force"
                    )
                ...
                if price is None or stop_price is None or stop_limit_price is None:
                    raise ValueError(
                        "TPSL orders require price, stop_price, and stop_limit_price"
                    )
                ...
                if fix_side == "2":  # Sell TPSL
                    if not (price > stop_price > stop_limit_price):
                        raise ValueError(
                            "For Sell TPSL: Price must be > StopPx and StopPx must be > StopLimitPx"
                        )
                else:  # Buy TPSL
                    if not (price < stop_price < stop_limit_price):
                        raise ValueError(
                            "For Buy TPSL: Price must be < StopPx and StopPx must be < StopLimitPx"
                        )

            await self.connection.send_msg(nos)
            logger.info(
                f"Placed {order_type} {side} order for {quantity} {symbol} "
    ...
    async def cancel_order(
        self, client_order_id: str, symbol: str, portfolio_id: Optional[str] = None
    ) -> str:
        """
        ...
        if self.test_mode:
            ...
            logger.info(
                f"Test mode: Simulating cancel for order {client_order_id} with cancel ID {cancel_client_order_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Sent cancel request for order {client_order_id} with cancel ID {cancel_client_order_id}"
            )
    ...
    async def modify_order(
        ...
        if self.test_mode:
            ...
            logger.info(
                f"Test mode: Simulating modify for order {original_client_order_id} with new ID {new_client_order_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Sent modify request for order {original_client_order_id} with new ID {new_client_order_id}"
            )
    ...
    async def mass_cancel_orders(
        ...
        if not self.authenticated or self.session_type != "order_entry":
            logger.error(
                "Cannot mass cancel orders: Not authenticated or wrong session type"
            )
            return ""
    ...
    async def request_positions(self) -> bool:
        ...
        if not self.authenticated or self.session_type != "order_entry":
            logger.error(
                "Cannot request positions: Not authenticated or wrong session type"
            )
            return False
    ...
    def _handle_security_list_request(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"SecurityListRequest: ID={security_req_id}, Type={security_list_request_type}"
            )
    ...
    def _handle_security_list(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"SecurityList: ID={security_req_id}, Result={security_request_result}, "
    ...
    def _handle_market_data_request(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"MarketDataRequest: ID={md_req_id}, Type={subscription_request_type}, "
    ...
    def _handle_market_data_request_reject(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            reason_desc = reason_map.get(
                md_req_rej_reason, f"Unknown ({md_req_rej_reason})"
            )

            logger.warning(
                f"MarketDataRequestReject: ID={md_req_id}, Reason={reason_desc}, Text={text}"
            )
    ...
    async def send_market_data_request(
        self, symbols: List[str], market_depth: int = 1, subscription_type: str = "1"
    ) -> str:
        """
    ...
    async def _wait_for_network_connection(self, timeout: int = 10) -> bool:
        """Wait until the underlying network connection is established."""
