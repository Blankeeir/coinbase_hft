...
class OrderBook:
    ...
    def __init__(self, symbol: str):
        ...
        self.trade_history: List[Tuple[float, float, str]] = []  # (timestamp, size, side)
        
        self.obi_history: List[Tuple[float, float]] = []  # (timestamp, OBI)
        self.volatility_history: List[Tuple[float, float]] = []  # (timestamp, volatility)
        self.trade_imbalance_history: List[Tuple[float, float]] = []  # (timestamp, trade_imbalance)
        self.vwap_distance_history: List[Tuple[float, float]] = []  # (timestamp, vwap_distance)
    ...
    def update_from_snapshot(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.debug(f"Updated order book for {self.symbol} from snapshot with "
    ...
    def update_from_incremental(self, message: FIXMessage) -> None:
        ...
        try:
            ...
            logger.debug(f"Updated order book for {self.symbol} incrementally with "
                        f"{len(self.bids)} bids and {len(self.asks)} asks")
    ...
    def get_weighted_mid_price(self, levels: int = 5) -> float:
        ...
        try:
            ...
            if total_bid_size > 0 and total_ask_size > 0:
                weighted_mid = (weighted_bid / total_bid_size + weighted_ask / total_ask_size) / 2
                return weighted_mid
    ...
    def get_vwap(self, window: int = 100) -> float:
        ...
        try:
            ...
            volume_price_sum = sum(size * self.get_mid_price() for _, size, _ in recent_trades)
...
class LimitOrderBook:
    ...
    def get_donchian_channel(self, window: int = 120) -> Tuple[float, float]:
        ...
        try:
            ...
            window_prices = [price for timestamp, price in self.price_history if timestamp >= window_start]
    ...
    def get_features(self) -> Dict[str, float]:
        ...
        try:
            ...
            upper_bound, lower_bound = self.get_donchian_channel(window=config.CHANNEL_WINDOW)
            features["upper_bound"] = upper_bound
            features["lower_bound"] = lower_bound
            features["channel_width"] = upper_bound - lower_bound
            features["channel_position"] = (self.get_mid_price() - lower_bound) / (upper_bound - lower_bound) if upper_bound > lower_bound else 0.5
    ...
    def _calculate_features(self) -> None:
        ...
        try:
            ...
            if len(self.trade_imbalance_history) > max_history:
                self.trade_imbalance_history = self.trade_imbalance_history[-max_history:]
...
class DataHandler:
    ...
    def is_breakout(self, symbol: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            upper_bound, lower_bound = order_book.get_donchian_channel(window=config.CHANNEL_WINDOW)
