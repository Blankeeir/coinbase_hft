...
class ChannelBreakoutStrategy:
    ...
    def update_model(self, features: Dict[str, float], label: int) -> None:
        ...
        try:
            ...
            if not hasattr(self.model, "classes_"):
                self.model.partial_fit(X_scaled, [label], classes=[-1, 0, 1])
            else:
                self.model.partial_fit(X_scaled, [label])

            self.n_samples += 1

            logger.debug("Incrementally updated model")
    ...
    def predict_direction(self, features: Dict[str, float]) -> Tuple[int, float]:
        ...
        """
        try:
            if self.model is None or self.n_samples < 100:
    ...
    def generate_signal(self) -> Tuple[str, float, Dict[str, Any]]:
        ...
        try:
            ...
            if self.prev_features is not None and self.prev_price > 0:
                label = (
                    1
                    if current_price > self.prev_price
                    else (-1 if current_price < self.prev_price else 0)
                )
                self.update_model(self.prev_features, label)
            ...
            # store features for next training step
